#!/usr/bin/env python3
"""Train a leakage-safe static-EEG 72-class semantic decoder.

Protocol:
  * train/validation use only objects 00--07;
  * validation is evaluated as a two-trial mean;
  * objects 08--09 remain untouched for the final test/inference stage.

The script is intentionally low-capacity and exposes controls for the current
overfitting problem: train representation, per-trial normalization, feature
noise, same-class latent consistency, validation metric selection, and label
smoothing. Inference is implemented separately in inference_neuralpp.py.
"""
    ...
    def __init__(self, input_dim=3200, hidden_dim=128, latent_dim=64,
        ...
        if self.use_transformer:
            ...
            if self.input_representation == "raw_waveform":
                ...
                if self.conv_pooled_steps <= 0:
                    raise ValueError("conv_pooled_steps must be positive")
                stem_channels = self.conv_channels // 2
                # The same temporal filters are applied to every channel.
                # This preserves the original 250-sample waveform until the
                # channel token has been constructed.
                self.temporal_encoder = nn.Sequential(
                    nn.Conv1d(1, stem_channels, kernel_size=15, stride=2, padding=7),
                    nn.GroupNorm(4, stem_channels),
                    nn.GELU(),
                    nn.Conv1d(stem_channels, self.conv_channels,
                              kernel_size=9, stride=2, padding=4),
                    nn.GroupNorm(8, self.conv_channels),
                    nn.GELU(),
                    nn.AdaptiveAvgPool1d(self.conv_pooled_steps),
                )
                self.token_count = CHANNELS
                encoded_dim = self.conv_channels * self.conv_pooled_steps
    ...
    def forward(self, x):
        if self.use_transformer:
            if self.input_representation == "raw_waveform":
                ...
                batch = x.shape[0]
                channels = x.reshape(batch * CHANNELS, 1, SAMPLES)
                tokens = self.temporal_encoder(channels).flatten(1)
                tokens = self.encoder(tokens).reshape(batch, CHANNELS, -1)
            else:
                tokens = x.reshape(x.shape[0], self.token_count, self.token_dim)
                tokens = self.encoder(tokens)
            tokens = self.transformer(tokens + self.position)
            latent = self.latent_projection(tokens.mean(dim=1))
        else:
...
def train(args):
    ...
    if getattr(args, "split_mode", "object") == "trial":
        # --- [신규] 트라이얼(Trial) 기반 80:20 무작위 분할 ---
        train_objects = list(range(8))
        val_objects = list(range(8))
        all_x_raw, all_y = make_train_features(
        ...
        train_x_raw, train_y = all_x_raw[indices[:split_idx]], all_y[indices[:split_idx]]
        val_x_raw, val_y = all_x_raw[indices[split_idx:]], all_y[indices[split_idx:]]
    else:
        # --- [기존] 객체(Object) 기반 분할 ---
    ...
    train_loader = DataLoader(
        FeatureDataset(train_x, train_y, noise_std=args.feature_noise_std, training=True),
    ...
    if getattr(args, "split_mode", "object") == "trial":
        ...
        val_x_raw_split, val_y_split, _ = make_trial_mean_features(
            arrays["train"], val_objects, input_norm=args.input_norm,
            input_representation=args.input_representation,
        )
        val_x = apply_scaler(val_x_raw_split, scaler_mean, scaler_scale)
        val_loader = DataLoader(
            FeatureDataset(val_x, val_y_split),
            batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=device.type == "cuda"
        )
    ...
    print(
        f"[data] train_objects={','.join(f'{i:02d}' for i in train_objects)} "
    ...
    checkpoint_config = {
        ...
        "include_trial_mean": args.train_mode == "both",
    ...
    summary = {
        "protocol": "static 1s EEG; object-disjoint validation; test inference is separate",
...
def main():
    ...
    parser.add_argument("--val_objects", default="06,07")
    ...
    parser.add_argument(
        ...
        help="raw_waveform keeps [64,250] EEG and encodes it with Conv1D.",
    )
    parser.add_argument(
        "--use_transformer", "--use-transformer", dest="use_transformer",
        action=argparse.BooleanOptionalAction, default=True,
        help="Add a channel-token Transformer after the encoder (use --no-use_transformer for legacy MLP).",
    )
    parser.add_argument("--token_dim", "--token-dim", type=int, default=50,
                        help="Values per token; 50 = 25 temporal means + 25 stds per channel.")
    ...
    parser.add_argument("--conv_channels", "--conv-channels", type=int, default=32)
    parser.add_argument("--conv_pooled_steps", "--conv-pooled-steps", type=int, default=4)
    parser.add_argument(
        "--train_mode", choices=["individual", "mean", "both"], default="individual",
        help="Training examples: individual trials, trial means, or both.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--split_mode", choices=["object", "trial"], default="object",
        help="object: 본 적 없는 객체로 검증 / trial: 전체 데이터 무작위 80:20 분할"
    )
