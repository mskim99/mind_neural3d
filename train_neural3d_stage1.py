...
# [수정 1] 새로운 데이터셋 임포트 (경로는 실제 구조에 맞게 변경)
...
if __name__ == '__main__':
    ...
    test_dataset = eeg_neuro3d_object(mode="test")
    ...
    # [수정 2] 데이터셋 로드 부분 변경
    # 사용자 환경에 맞춰 data_path와 sub_list를 설정해야 합니다.
    data_path = "/data/jionkim/neuro_3D/"
    sub_list = [args.sub_id]  # 예: "sub01"

    train_dataset = AllDataFeatureTwoEEG(
        data_path=data_path,
        sub_list=sub_list,
        train=True,
        num_frames=16  # 16프레임 추출
    )
