...
# =========================================================================
# 3. Semantic-Prior-Conditioned MVDiffusion Pipeline
# =========================================================================
class MVDiffusion(nn.Module):
    def __init__(self, args, stable_diffusion_config, drop_cond_prob=0.1, fmri_encoder_config=None, logdir=None, num_classes=72, cls_label_smoothing=0.05):
        ...
        # =========================================================
        # Semantic-prior-conditioned generation
        # =========================================================
        #
        # We intentionally bypass the high-capacity shared/GCN/semantic path for
        # generation.  Only the already-selected TemporalStem is used, frozen.
        #
        # EEG -> frozen TemporalStem -> time mean [2048]
        #     -> fixed train-only PCA [K]
        #     -> 72-way category distribution
        #     -> mixture of NON-TRAINABLE category text prototypes [1024]
        #     -> tiny gated EEG residual
        #     -> Zero123++ semantic cross-attention
        #
        # Diffusion gradients NEVER update the category classifier.  The
        # classifier is optimized only by category CE; diffusion can update only
        # the tiny residual/gate, semantic_to_cross, LoRA, and optional weak
        # spatial branch.
        self.semantic_prior_pca_dim = int(
            getattr(args, "semantic_prior_pca_dim", 512)
        )
        self.semantic_prior_residual_rank = int(
            getattr(args, "semantic_prior_residual_rank", 64)
        )
        self.semantic_prior_residual_scale = float(
            getattr(args, "semantic_prior_residual_scale", 0.05)
        )
        self.semantic_prior_temperature = float(
            getattr(args, "semantic_prior_temperature", 0.5)
        )
        self.semantic_prior_spatial_scale = float(
            getattr(args, "semantic_prior_spatial_scale", 0.0)
        )
        self.semantic_prior_label_smoothing = float(
            getattr(args, "semantic_prior_label_smoothing", 0.05)
        )

        # sklearn PCA transform:
        #   z = (x - mean) @ components.T
        # Shapes are fixed at construction for checkpoint compatibility.
        self.register_buffer(
            "semantic_prior_pca_mean",
            torch.zeros(2048, dtype=torch.float32),
            persistent=True,
        )
        self.register_buffer(
            "semantic_prior_pca_components",
            torch.zeros(
                self.semantic_prior_pca_dim, 2048, dtype=torch.float32
            ),
            persistent=True,
        )
        self.register_buffer(
            "semantic_prior_pca_ready",
            torch.tensor(False, dtype=torch.bool),
            persistent=True,
        )

        # Separate normalization paths keep the classifier strictly CE-only:
        # diffusion gradients cannot alter the classifier normalization.
        self.semantic_prior_classifier_norm = nn.LayerNorm(
            self.semantic_prior_pca_dim
        )
        self.semantic_prior_residual_norm = nn.LayerNorm(
            self.semantic_prior_pca_dim
        )
        self.semantic_prior_classifier = nn.Linear(
            self.semantic_prior_pca_dim, self.num_classes
        )
        self.semantic_prior_residual = nn.Sequential(
            nn.Linear(
                self.semantic_prior_pca_dim,
                self.semantic_prior_residual_rank,
            ),
            nn.GELU(),
            nn.Linear(self.semantic_prior_residual_rank, 1024),
        )
        self.semantic_prior_gate = nn.Linear(
            self.semantic_prior_pca_dim, 1
        )

        # Optional spatial modulation. Default scale=0: semantic-only generation.
        self.semantic_prior_spatial = nn.Sequential(
            nn.LayerNorm(self.semantic_prior_pca_dim),
            nn.Linear(self.semantic_prior_pca_dim, 1024),
            nn.GELU(),
            nn.Linear(1024, 1024),
        )
    ...
    def train(self, mode=True):
        ...
        if mode and getattr(self, "training_stage", None) in {
            "joint", "prior_generation"
        }:
            # Frozen EEG feature source. In particular, TemporalStem BatchNorm
            # running statistics must never drift during prior-generation training.
    ...
    def set_semantic_prior_pca(self, mean, components):
        """Install a STRICT train-only sklearn PCA transform."""
        mean = torch.as_tensor(
            mean, dtype=torch.float32,
            device=self.semantic_prior_pca_mean.device,
        ).reshape(-1)
        components = torch.as_tensor(
            components, dtype=torch.float32,
            device=self.semantic_prior_pca_components.device,
        )

        if tuple(mean.shape) != (2048,):
            raise ValueError(
                f"Expected PCA mean [2048], got {tuple(mean.shape)}"
            )
        expected = (self.semantic_prior_pca_dim, 2048)
        if tuple(components.shape) != expected:
            raise ValueError(
                f"Expected PCA components {expected}, "
                f"got {tuple(components.shape)}"
            )

        self.semantic_prior_pca_mean.copy_(mean)
        self.semantic_prior_pca_components.copy_(components)
        self.semantic_prior_pca_ready.fill_(True)

    def set_semantic_prior_training(self):
        """
        One-stage generation optimization.

        Frozen:
          - entire EEG encoder, including TemporalStem/BN
          - fixed category prototype geometry
          - VAE / text encoder / base UNet

        CE only:
          - semantic_prior_classifier

        Diffusion:
          - semantic_prior_residual + gate
          - semantic_to_cross
          - LoRA
          - optional semantic_prior_spatial + variation_to_latent when
            semantic_prior_spatial_scale > 0
        """
        self.training_stage = "prior_generation"
        self.requires_grad_(False)
        self.fixed_text_prototypes.requires_grad_(False)

        self.semantic_prior_classifier_norm.requires_grad_(True)
        self.semantic_prior_classifier.requires_grad_(True)
        self.semantic_prior_residual_norm.requires_grad_(True)
        self.semantic_prior_residual.requires_grad_(True)
        self.semantic_prior_gate.requires_grad_(True)
        self.semantic_to_cross.requires_grad_(True)

        for name, p in self.unet.named_parameters():
            p.requires_grad_(name in self._unet_lora_trainable_names)

        if self.semantic_prior_spatial_scale > 0:
            self.semantic_prior_spatial.requires_grad_(True)
            self.variation_to_latent.requires_grad_(True)

        # Absolutely no EEG/BN updates.
        ...
        print(
            "[stage=prior_generation] trainable params: "
            f"{trainable / 1e6:.2f}M / {total / 1e6:.2f}M"
        )

    @torch.no_grad()
    def extract_semantic_prior_temporal(self, eeg):
        """
        Frozen signal feature used to fit train-only PCA.

        EEG [B,64,600]
          -> TemporalStem per electrode [B,64,32,149]
          -> mean over time
          -> [B,2048]
        """
        self.fmri_encoder.temporal_stem.eval()

        with torch.autocast("cuda", enabled=False):
            eeg = eeg.float()
            B, E, T = eeg.shape
            x = eeg.reshape(B * E, 1, T)
            x = self.fmri_encoder.temporal_stem(x)
            x = x.reshape(B, E, x.shape[1], x.shape[2])
            x = x.mean(dim=-1).reshape(B, -1)

        if x.shape[-1] != 2048:
            raise RuntimeError(
                f"Expected frozen temporal feature 2048-D, got {x.shape[-1]}"
            )
        return x.float()

    def _semantic_prior_feature(self, eeg):
        if not bool(self.semantic_prior_pca_ready.item()):
            raise RuntimeError(
                "Semantic-prior PCA is not installed. "
                "Call set_semantic_prior_pca() before training/inference."
            )

        # TemporalStem is frozen, so do not build a useless autograd graph.
        with torch.no_grad():
            raw = self.extract_semantic_prior_temporal(eeg)

        with torch.autocast("cuda", enabled=False):
            centered = raw.float() - self.semantic_prior_pca_mean.view(1, -1)
            feat = centered @ self.semantic_prior_pca_components.t()
        return feat.float()

    def semantic_prior_forward(
        self,
        eeg,
        cls_target=None,
        teacher_mix=0.0,
        ...
        """
        Build semantic-prior conditioning.

        teacher_mix is TRAINING-ONLY:
          1.0 -> use true category anchor for diffusion
          0.0 -> use predicted soft category distribution

        At inference always use teacher_mix=0.
        """
        feat = self._semantic_prior_feature(eeg)

        with torch.autocast("cuda", enabled=False):
            cls_feat = self.semantic_prior_classifier_norm(feat.float())
            residual_feat = self.semantic_prior_residual_norm(feat.float())

            logits = self.semantic_prior_classifier(cls_feat)
            probs = F.softmax(
                logits / self.semantic_prior_temperature, dim=-1
            )
            ...
            if cls_target is not None:
                cls_loss = F.cross_entropy(
                    logits,
                    cls_target,
                    label_smoothing=self.semantic_prior_label_smoothing,
                )

            probs_for_diff = (
                probs.detach()
                if detach_predicted_prior_for_diffusion
                else probs
            )

            mix = float(max(0.0, min(1.0, teacher_mix)))
            if cls_target is not None and mix > 0:
                teacher = F.one_hot(
                    cls_target, num_classes=self.num_classes
                ).float()
                probs_for_diff = (
                    mix * teacher
                    + (1.0 - mix) * probs_for_diff
                )

            prior_anchor = F.normalize(
                probs_for_diff @ self.fixed_text_prototypes.float(),
                dim=-1,
            )

            residual_direction = F.normalize(
                self.semantic_prior_residual(residual_feat),
                dim=-1,
            )
            gate = self.semantic_prior_residual_scale * torch.sigmoid(
                self.semantic_prior_gate(residual_feat)
            )

            semantic_prior = F.normalize(
                prior_anchor + gate * residual_direction,
                dim=-1,
            )
    ...
    def build_semantic_prior_zero123_condition(
        self,
        eeg,
        cls_target=None,
        teacher_mix=0.0,
        ...
        """
        Semantic prior -> Zero123++ conditioning.

        Default is semantic-only: spatial cond_lat = 0. This deliberately
        prevents the old high-capacity variation branch from memorizing seen
        objects. A weak spatial path can be enabled with
        semantic_prior_spatial_scale > 0.
        """
        prior = self.semantic_prior_forward(
            eeg,
        ...
        if drop_condition:
            ...
            return prior, semantic_cond, empty_prompt, cond_latents

        with torch.autocast("cuda", enabled=False):
            semantic_cond = self.semantic_to_cross(
                prior["semantic_prior"].float()
            ).unsqueeze(1)

            if self.semantic_prior_spatial_scale > 0:
                spatial_embed = self.semantic_prior_spatial(
                    self.semantic_prior_residual_norm(
                        prior["feature"].float()
                    )
                )
                cond_latents = (
                    self.semantic_prior_spatial_scale
                    * self.variation_to_latent(spatial_embed)
                )
            else:
                cond_latents = torch.zeros(
                    (B, 4, 64, 64),
    ...
    def forward(
        ...
        if stage == "prior_generation":
            if "cls_index" not in batch:
                raise KeyError(
                    "prior_generation requires explicit batch['cls_index']."
                )
            ...
            drop_condition = bool(
                torch.rand((), device=cond_eeg.device)
                < self.drop_cond_prob
            )
            ...
            (
                ...
                prompt_embeds,
                eeg_cond_latents,
            ) = self.build_semantic_prior_zero123_condition(
                cond_eeg,
                cls_target=cls_target,
                teacher_mix=teacher_mix,
            ...
            return {
                "diff_loss": diff_loss,
                "prior_cls_loss": prior["cls_loss"],
                "prior_top1": prior["top1"],
                "prior_entropy": prior["entropy"],
                "prior_gate": prior["gate"].mean(),
                "teacher_mix": diff_loss.new_tensor(float(teacher_mix)),
            }
    ...
    def encode_embed_fmri_condition_fmri(self, eeg, drop_condition=False):
        ...
        Semantic-prior checkpoints use the new prior path automatically.
        Older checkpoints without installed PCA keep the legacy path.
        """
        if bool(self.semantic_prior_pca_ready.item()):
            (
                _,
                semantic_cond,
                encoder_hidden_states,
    ...
    def get_loss(self, batch, stage="joint", **kwargs):
        losses = self.forward(
            ...
            teacher_mix=kwargs.get("teacher_mix", 0.0),
        )
        if stage == "prior_generation":
            return (
                losses["diff_loss"],
                losses["prior_cls_loss"],
                losses["prior_gate"],
            )
        return (
            losses["diff_loss"],
