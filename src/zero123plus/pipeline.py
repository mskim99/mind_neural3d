...
class Zero123PlusPipeline(diffusers.StableDiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs
        ...
        # latents = self.fmri_encoder.forward_encoder_wo_pred(fmri.to(self.vae.dtype)) # [B*6, 257, 1024]
        # global_embeds, latents = latents[:,:1,:], latents[:,1:,:]
        
        # latents = latents.view(B,32,32,2,2,256).permute(0,1,3,2,4,5)
        # latents = latents.reshape(B, 64, 64, 256)
        # cond_lat = self.fmri_encoder.aggregation(latents).permute(0, 3, 1, 2)


        # if guidance_scale > 1:
        #     latents = self.fmri_encoder.forward_encoder_wo_pred(torch.zeros_like(fmri)) # [6, 257, 1024]
        #     global_embeds, latents = latents[:,:1,:], latents[:,1:,:]
            
        #     latents = latents.view(B,32,32,2,2,256).permute(0,1,3,2,4,5)
        #     latents = latents.reshape(B, 64, 64, 256)
        #     negative_lat = self.fmri_encoder.aggregation(latents).permute(0, 3, 1, 2)
        #     cond_lat = torch.cat([negative_lat, cond_lat])
            
        latents = self.fmri_encoder.forward_encoder_wo_pred(fmri.to(self.vae.dtype).view(-1, 1, 256, 256)) # [B*6, 257, 1024]
        ...
        if guidance_scale > 1:
            latents = self.fmri_encoder.forward_encoder_wo_pred(torch.zeros_like(fmri).view(-1, 1, 256, 256)) # [6, 257, 1024]
            embed_fmri, latents = latents[:,:1,:], latents[:,1:,:]
            latents = latents.view(B, 6, 16, 16, 4, 4, 64).permute(0,1,2,4,3,5,6)
            latents = latents.reshape(B, 6, 16 * 4, 16 * 4, 64)
            latents = latents.permute(0, 2, 3, 1, 4).reshape(B, 64, 64, 64 * 6)
